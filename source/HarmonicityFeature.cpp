...
namespace Sirens {
	HarmonicityFeature::HarmonicityFeature(int history_size, int spectrum_size, int sample_rate) : Feature(history_size) {
		initialized = false;
	...
	HarmonicityFeature::~HarmonicityFeature() {
		freeMemory();
	}

	void HarmonicityFeature::setSpectrumSize(int spectrum_size) {
		spectrumSize = spectrum_size;
		
		initialized = false;
	}
	
	void HarmonicityFeature::setSampleRate(int sample_rate) {
		sampleRate = sample_rate;
		
		initialized = false;
	}
	
	void HarmonicityFeature::setAbsThreshold(double abs_threshold) {
		absThreshold = abs_threshold;
		
		initialized = false;
	}
	
	void HarmonicityFeature::setThreshold(double new_threshold) {
		threshold = new_threshold;
		
		initialized = false;
	}
	
	void HarmonicityFeature::setSearchRegionLength(int search_region_length) {
		searchRegionLength = search_region_length;
		
		initialized = false;
	}
	
	void HarmonicityFeature::setMaxPeaks(int max_peaks) {
		maxPeaks = max_peaks;
		
		initialized = false;
	}
	
	void HarmonicityFeature::setLPFCoefficient(double lpf_coefficient) {
	...
	int HarmonicityFeature::getSpectrumSize() {
		return spectrumSize;
	}
	
	int HarmonicityFeature::getSampleRate() {
		return sampleRate;
	}
	
	double HarmonicityFeature::getAbsThreshold() {
		return absThreshold;
	}
	
	double HarmonicityFeature::getThreshold() {
		return threshold;
	}
	
	int HarmonicityFeature::getSearchRegionLength() {
		return searchRegionLength;
	}
	
	int HarmonicityFeature::getMaxPeaks() {
		return maxPeaks;
	}
	
	double HarmonicityFeature::getLPFCoefficient() {
		return lpfCoefficient;
	}
	
	void HarmonicityFeature::initialize() {	
	...
	void HarmonicityFeature::freeMemory() {
	...
	void HarmonicityFeature::peakPick() {		
	...
	double HarmonicityFeature::goldsteinGaussian(double x1, double x2, int n1, int n2, double f0, double k) {
		double mu1 = f0 * n1;
		double mu2 = f0 * n2;
	...
	void HarmonicityFeature::goldsteinCalc() {
	...
	void HarmonicityFeature::performCalculation() {
		peakPick();
